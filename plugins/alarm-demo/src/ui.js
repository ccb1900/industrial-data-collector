...
export default function register(m) {
                    ...
                    series: [
                        {
                            ...
                            type: "line",
                            smooth: true,
                            data: dates.map((d) => byDate.get(d).records),
                        },
                        {
                            name: "失败文件",
                            type: "bar",
    ...
    function AlarmConsole() {
        ...
        const load = React.useCallback(() => {
            setLoading(true)
            Promise.all([
                api.hubQuery("failures"),
                api.hubQuery("collections"),
                api.hubQuery("schedule"),
            ])
                .then(([failures, cols, sched]) => {
                    const rows = (Array.isArray(failures) ? failures : []).map(
                        (f, i) => ({
                            key: String(i),
                            level:
                                Number(f.attempts ?? 0) >= 3
                                    ? "critical"
                                    : "warning",
                            ...f,
                        }),
        ...
        const columns = [
            {
                ...
                render: (v) =>
                    e(
                        ...
                        {
                            style: {
                                color: (LEVELS[v] ?? {}).color,
                                fontWeight: 600,
                            },
                        },
                        (LEVELS[v] ?? {}).label ?? String(v ?? "—"),
                    ),
            },
        ...
        return e(
            ...
            e(
                ...
                card(
                    "n",
                    "下次采集",
                    (schedule.next ?? "—").replace("T", " ").slice(0, 16),
                    "#4d6bfe",
                ),
            ),
            e(
                ...
                {
                    style: {
                        border: "1px solid var(--line-soft, #d9dee8)",
                        borderRadius: 10,
                        padding: "8px 10px",
                    },
                },
            ...
            e(
                ...
                e(
                    ...
                    e(
                        antd.Button,
                        { size: "small", onClick: load, loading },
                        "刷新",
                    ),
                    e(
                        antd.Button,
